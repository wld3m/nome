...
describe("migrationRunner/constants — large-table integrity", () => {
  it("RENAMED_MIGRATION_COMPATIBILITY has 18 well-formed entries", () => {
    assert.equal(RENAMED_MIGRATION_COMPATIBILITY.length, 18);
  ...
  it("RENAMED_MIGRATION_COMPATIBILITY spot-checks the boundary renames", () => {
    ...
    assert.deepEqual(
      ...
      ["123", "124", "125", "126", "127", "128", "131", "133"]
    );
    assert.ok(devin.every((e) => e.toVersion === "134"));
  });
